...
const blog_storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    const blog_timestamp = Date.now();
    const blog_extension = path.extname(file.originalname);
    const blog_newFilename = `${blog_timestamp}${blog_extension}`;
    cb(null, blog_newFilename);
  }
});
...
const aStoreBlogPost = asyncHandler(async (req, res) => {
  ...
  if (blog_post) {
    res.status(201).json({ message: `Blog successfully stored` });
  } else {
    res.status(400).json({ message: `Error occurred` });
  }
});
...
const aStorePlaylistAudio = asyncHandler(audio_upload.single('audioFile'), async (req, res) => {
  console.log(req.body.audioFile); // Change from req.audioFile to req.file
  try {
    if (req.body.audioFile) {
      res.status(201).json({ message: 'Not Empty' });
    } else {
